...
class AdaBoost(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        for t in range(self.iterations_):
            f_hat_t = self.wl_()
            f_hat_t.fit(X, y * self.D_)
            y_hat_t = f_hat_t.predict(X)
            self.models_[t] = f_hat_t
            epsilon_t = np.sum(self.D_[y != y_hat_t])
            w_t = 0.5 * np.log(1 / epsilon_t - 1)
            self.weights_[t] = w_t
            self.D_ = self.D_ * np.exp(-y * w_t * y_hat_t)
    ...
    def partial_predict(self, X: np.ndarray, T: int) -> np.ndarray:
        ...
        for t in range(T):
            weighted_predictions += self.weights_[t] \
